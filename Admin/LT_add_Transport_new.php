				...
				<div class="card-header">
					<div class="input-group input-group-sm">
						<div class="input-group-append" style="text-align: left;">
							<div style="padding-right: 5px;"> <button type="button" onclick="LT_itinerary(25,<?php echo $row_data['id']  ?>,<?php echo $row_data['master_id'] ?>)" class="btn btn-primary btn-sm"><i class="fa fa-arrow-left"></i></button></div>
							<div style="padding-right: 5px;"> <button type="button" onclick="LT_itinerary(24,<?php echo $_POST['id'] ?>, 0)" class="btn btn-primary btn-sm"><i class="fas fa-sync"></i></button></div>
...
<script>
	...
	function fungsi_add_flight3(x, y, z) {
		let formData = new FormData();
		var copy_id = y;
		var master_id = z;
		var grub_id = x;
		...
		var cek_hari = 0;
		for (var i = 1; i < loop; i++) {
			var id_fl = $("input[name=id_fl" + i + "]").val();
			var urutan = $("input[name=f_urutan" + i + "]").val();
			var hari = $("input[name=f_hari" + i + "]").val();

			if (id_fl != "") {
				// formData.append('kode[]', id_fl);
				hasil.push(id_fl);
			}
			if (urutan != "") {
				formData.append('urutan[]', urutan);
