...
function get_total($datareq)
{
    ...
    $flight_post_id = isset($datareq['flight']) ? $datareq['flight'] : '';
    $date_post = isset($datareq['date']) ? $datareq['date'] : '';
    $grub_id = isset($datareq['grub_id']) ? $datareq['grub_id'] : '';
    ...
    if ($check_id == '1') {
        ...
        $data = [];
        if ($row_master['landtour'] != "undefined") {
            $query = "SELECT * FROM  LT_select_PilihHTL where master_id='" . $master_id . "' && copy_id='" . $copy_id . "'";
            $rs = mysqli_query($con, $query);
            $row = mysqli_fetch_assoc($rs);
        ...
        $query_plane = "SELECT * FROM  LT_add_transport_baru where master_id='" . $master_id . "' && copy_id='" . $copy_id . "' && type='2' && grub_id='" . $grub_id . "'";
        $rs_plane = mysqli_query($con, $query_plane);
        ...
        $query_plane = "SELECT * FROM LT_add_transport_baru where master_id='" . $master_id . "' && copy_id='" . $copy_id . "' && grub_id='" . $grub_id . "' && type='1'";
        $rs_plane = mysqli_query($con, $query_plane);
        $price = 0;
        $data = [];
        $bagasi_price = 0;
        while ($row_plane = mysqli_fetch_array($rs_plane)) {
            ...
            $queryflight = "SELECT * FROM  LTP_route_detail where id='" . $row_plane['transport'] . "'";
        ...
        return json_encode(array("adt" => $bagasi_price, "chd" => $bagasi_price, "inf" => 0, "sgl" => $bagasi_price, "detail" => $query_plane), true);
    } else if ($check_id == '24') {
        ...
        return json_encode(array("adt" => $vttpax, "chd" => $vttpax, "inf" => $vttpax, "sgl" => $vttpax, "detail" => $data, "breakdown" => $arr_breakdown, "break_cost_tl" => $arr_break_costtl, "feetl_cover" => $arr_feetl_cover, "costtl_cover" => $arr_ct_cover, "grand" => $grand), true);
        // return json_encode(array("adt" => 1, "chd" => 1, "inf" => 1, "sgl" => 1, "detail" => $data), true);
    } else if ($check_id == '36') {
        ...
        $query_hotel_data = "SELECT * FROM LTHR_add_hotel WHERE copy_id='" . $copy_id . "' and master_id='" . $master_id . "'";
        ...
        while ($row_hotel_data = mysqli_fetch_array($rs_hotel_data)) {
            ...
            if ($row_hotel_data['rate'] == '1') {
                ...
                $gt = $gt + ($result_rate2['price'] / $row_hlt['occ']);
            } else {
                $data = array(
                    "kurs" =>  $row_hlt['kurs'],
        ...
        return json_encode(array("adt" => $gt, "chd" => 0, "inf" => 0, "sgl" => $gt, "detail" => 0), true);
    } else {
        return json_encode(array("adt" => 0, "chd" => 0, "inf" => 0, "sgl" => 0, "detail" => 0), true);
    }
}
...
function get_kurs_landtrans($d)
{
    ...
    $row = mysqli_fetch_array($rs);
    if (!isset($row['id'])) {
        return json_encode(array("status" => "data Kurs tidak Tersedia", "rate" => 0, "oneway" => 0, "twoway" => 0, "hd1" => 0, "hd2" => 0, "fd1" => 0, "fd2" => 0, "kaisoda" => 0, "luarkota" => 0), true);
    } else {
        if ($kurs == "IDR") {
            return json_encode(array("status" => "kurs sama", "rate" => 1, "oneway" => $oneway, "twoway" => $twoway, "hd1" => $hd1, "hd2" => $hd2, "fd1" => $fd1, "fd2" => $fd2, "kaisoda" => $kaisoda, "luarkota" => $luarkota), true);
        } else {
            if ($fd2 == '0') {
                return json_encode(array("status" => "nominal 0", "data" => $fd2), true);
            } else {
                // $price = $nominal * $row['jual'];
                ...
                return json_encode(array("status" => "Success", "rate" => $row['jual'], "oneway" => $oneway_idr, "twoway" => $twoway_idr, "hd1" => $hd1_idr, "hd2" => $hd2_idr, "fd1" => $fd1_idr, "fd2" => $fd2_idr, "kaisoda" => $kaisoda_idr, "luarkota" => $luarkota_idr), true);
            }
        }
    }
}
    ...
    foreach ($adm_inc as $val_tmp) {
        ...
        if (isset($row_tmp['id'])) {
            if ($row_tmp['price'] != '0') {
                ...
                $cek_tmp .= $rs_adt_kurs['data'] . " - ";
                $val = $row_tmp['tempat'];
            }
        }
    }
    return json_encode(array("adt" => $adt_tmp, "chd" =>  $adt_tmp, "inf" => $adt_tmp, "sgl" => $adt_tmp, "detail" => $cek_tmp, "tmp" => $val), true);
}
...
function get_hotel_lt_price($x)
{
    ...
    return json_encode(array("status" =>  "ok", "price" => $gprice), true);
}
