    ...
    if ($check_id == '1') {
        ...
        if ($row_master['landtour'] != "undefined") {
            ...
            if ($row['hotel_id'] != "") {
                ...
                // $agn_s = $row['adt'] * ($row_profit['sub_agent'] / 100);
                // $ste = $row_profit['staff_eks'];
                $nom = $row_profit['nominal'];
        ...
        $query_listtmp = "SELECT * FROM   LT_add_listTmp where tour_id='" . $master_id . "'";
        $rs_listtmp = mysqli_query($con, $query_listtmp);
        $data = [];
        $adt = 0;
        $chd = 0;
        $inf = 0;
        while ($row_listtmp = mysqli_fetch_array($rs_listtmp)) {
            $query_tmp = "SELECT tempat2,kurs,price,chd,infant FROM  List_tempat where id='" . $row_listtmp['tempat'] . "'";
            ...
            if ($row_tmp['inf'] != 0) {
                ...
                $inf = $inf +  $rs_inf_kurs['data'];
            }
            array_push($data, $row_tmp['tempat2']);
        }
        return json_encode(array("adt" => $adt, "chd" => $chd, "inf" => $inf, "sgl" => $adt, "detail" => $data), true);
    } else if ($check_id == '18') {
        $query_plane = "SELECT * FROM  LT_add_transport where master_id='" . $master_id . "' && copy_id='" . $copy_id . "' && type='2'";
        $rs_plane = mysqli_query($con, $query_plane);
        ...
        $query_train = "SELECT * FROM  LT_add_transport where master_id='" . $master_id . "' && copy_id='" . $copy_id . "' && type='4'";
        ...
        // array_push($data, "P COVER : " .$pax_covery." * ".$lt_price);
