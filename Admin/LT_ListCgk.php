...
session_start();
// $arr_visa = [];
// $query_visa = "SELECT * FROM Visa2 where kota='SURABAYA' order by id ASC";
// $rs_visa = mysqli_query($con, $query_visa);
// while ($row_visa = mysqli_fetch_array($rs_visa)) {
//     array_push($arr_visa, array(
//         "id" => $row_visa['id'],
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title" style="font-weight:bold;">Landtour CGK List</h3>
                ...
                <div class="card-body table-responsive p-0">
                    <?php
                    $query = "SELECT * FROM  LTSUB_itin where cabang='4' order by id ASC";
                    $rs = mysqli_query($con, $query);
                    $no = 1;
                    ?>
                    <div style="padding: 20px;">
                        <table id="tb-pt-cgk" class="table table-striped table-bordered" style="width:100%; font-size: 12px;">
                            <thead>
                                <tr>
                                    <th>Id</th>
                                    <th>Nama</th>
                            ...
                            <tbody>
                                ...
                                while ($row = mysqli_fetch_array($rs)) {
                                    ...
                                    // $query_date = "SELECT * FROM  LT_Date_list where master_id='" . $row['master_id'] . "' && copy_id='" . $row['id'] . "' && ket='1'";
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            ?>
                                            <!-- <a class="btn btn-warning btn-sm tip" onclick="LT_itinerary(14,<?php echo $row['id'] ?>,<?php echo $row['master_id'] ?>)" title="Print Semua Hotel"><i class="fa fa-print"></i></a> -->
                                            <!-- <a class="btn btn-danger btn-sm tip" onclick="LT_itinerary(17,<?php echo $row['id'] ?>,<?php echo $row['master_id'] ?>)" title="Print Custom"><i class="fa fa-print"></i></a>
                                        <a class="btn btn-info btn-sm tip" onclick="LT_itinerary(26,<?php echo $row['id'] ?>,<?php echo $row['master_id'] ?>)" title="Print Custom baru"><i class="fa fa-print"></i></a> -->
                                            ...
                                            <!-- <a class="btn btn-success btn-sm tip" onclick="LT_itinerary(34,<?php echo $row['id'] ?>,<?php echo $row['master_id'] ?>)" title="Print Custom"><i class="fa fa-print"></i></a> -->
                                            <?php
                                            if ($rowStaff2['cabang'] == '4') {
                                            ?>
                                                <!-- <a class="btn btn-danger btn-sm tip" onclick="LT_itinerary(8,<?php echo $row['id'] ?>,0)" title="Tambah Hari"><i class="far fa-calendar-plus"></i></a> -->
                                                ...
                                                <!-- <a class="btn btn-warning btn-sm tip" onclick="LT_itinerary(19,<?php echo $row['id'] ?>,<?php echo $row['master_id'] ?>)" title="Add Tgl Berangkat"><i class="far fa-calendar-check"></i></a>
                                            <a class="btn btn-primary btn-sm tip" onclick="LT_itinerary(10,<?php echo $row['id'] ?>,<?php echo $row['master_id'] ?>)" title="Add Flight"><i class="fa fa-plane"></i></a> -->
                                                ...
                                                ?>
                                                <!-- <a class="btn btn-info btn-sm" data-toggle="modal" data-target="#exampleModal<?php echo $row['id']  ?>"><i class="fas fa-users"></i></i></a> -->
                                                <?php
                                                if ($row['landtour'] == "undefined") {
                                                ?>
                                                ...
                                                ?>
                                                <!-- <a class="btn btn-primary btn-sm tip" onclick="LT_itinerary(12,<?php echo $row['id'] ?>,<?php echo $row['master_id'] ?>); showLoading();" title="Cek Price">
                                                <i class="fas fa-wallet"></i></a>
                                            <a class="btn btn-danger btn-sm tip" onclick="LT_itinerary(121,<?php echo $row['id'] ?>,<?php echo $row['master_id'] ?>); showLoading();" title="Cek Price">
                                                <i class="fas fa-wallet"></i></a> -->
                                                <?php
                                                $query_cek_visa = "SELECT id FROM Visa_add where  master_id='" . $row['master_id'] . "' && copy_id='" . $row['id'] . "'";
                                                $rs_cek_visa = mysqli_query($con, $query_cek_visa);
                                                $row_cek_visa = mysqli_fetch_array($rs_cek_visa);
                                                if ($row_cek_visa['id'] != "") {
                                                ?>
                                                    <a class="btn btn-success btn-sm tip" data-toggle="modal" data-target="#visa" data-id="<?php echo $row['id']  ?>" data-master="<?php echo $row['master_id'] ?>" title="Add Visa"><i class="fas fa-file"></i></a>
                                                <?php
                                                } else {
                                                ?>
                                                    <a class="btn btn-warning btn-sm tip" data-toggle="modal" data-target="#visa" data-id="<?php echo $row['id']  ?>" data-master="<?php echo $row['master_id'] ?>" title="Add Visa"><i class="fas fa-file"></i></a>
                                                <?php
                                                }
                                                ?>
                                                <a class="btn btn-primary btn-sm tip" onclick="LT_itinerary(18,<?php echo $row['id'] ?>,0)" title="Edit Judul"><i class="fa fa-edit"></i></a>
                                                <!-- <a class="btn btn-primary btn-sm tip" data-toggle="modal" data-target="#linkCustom" data-id="<?php echo $row['id']  ?>" data-master="<?php echo $row['master_id'] ?>" title="Edit Rute"><i class="fa fa-tools"></i></a> -->
...
<script>
    ...
    function add_visa() {
        let formData = new FormData();
        var visa = document.getElementById('negara').value;
