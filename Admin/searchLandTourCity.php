          ...
          <div class='col-12'>
            <div class='card'>
              <div class='card-header'>
                <h3 class='card-title' style='font-weight:bold;'>TOUR PACKAGE</h3>
            ...
            <div id='filterMenu' name='filterMenu' >
              ...
              <div>";
                ...
                if($_POST['name']!=0){
                  $query_country2 = "SELECT * FROM city WHERE id = ".$_POST['name'];
                  $rs_country2=mysqli_query($con,$query_country2);
                }else{
                  $query_country2 = "SELECT * FROM city ";
                  $rs_country2=mysqli_query($con,$query_country2);
                }
               ...
               echo "<th>Tour Name</th>
                ...
                <tbody id='myTable'>";
                 while($row_country2 = mysqli_fetch_array($rs_country2) and $countx!=1){

                if($_POST['tourtype']!=''){

                   if($_POST['minperson']!=''){
                     if($_SESSION['type']!=1 and $_SESSION['type']!=2 and $_SESSION['type']!=5 and $_SESSION['staff']!="Joana" and $_SESSION['type']!=6){
                        $query = "SELECT * FROM tour_package WHERE staff=".$_SESSION['staff_id']." AND tour_type LIKE '".$_POST['tourtype']."' AND minperson=".$_POST['minperson']." AND city LIKE '%".$row_country2['id']."%' ORDER BY id DESC";
                      }else{
                        $query = "SELECT * FROM tour_package WHERE tour_type LIKE '".$_POST['tourtype']."' AND minperson=".$_POST['minperson']." AND city LIKE '%".$row_country2['id']."%' ORDER BY id DESC";
                      }
                  }else{
                    $query = "SELECT * FROM tour_package WHERE tour_type LIKE '".$_POST['tourtype']."' AND city LIKE '%".$row_country2['id']."%' ORDER BY id DESC";
                  }
                  
                }elseif($_POST['minperson']!=''){

                   if($_SESSION['type']!=1 and $_SESSION['type']!=2 and $_SESSION['type']!=5 and $_SESSION['staff']!="Joana" and $_SESSION['type']!=6){
                      $query = "SELECT * FROM tour_package WHERE staff=".$_SESSION['staff_id']." AND minperson=".$_POST['minperson']." AND city LIKE '%".$row_country2['id']."%' ORDER BY id DESC";
                    }else{
                      $query = "SELECT * FROM tour_package WHERE minperson=".$_POST['minperson']." AND city LIKE '%".$row_country2['id']."%' ORDER BY id DESC";
                    }
                  
                }else{
                     if($_SESSION['type']!=1 and $_SESSION['type']!=2 and $_SESSION['type']!=5 and $_SESSION['staff']!="Joana" and $_SESSION['type']!=6){
                      $query = "SELECT * FROM tour_package WHERE staff=".$_SESSION['staff_id']." AND city LIKE '%".$row_country2['id']."%' ORDER BY id DESC";
                    }else{
                      $query = "SELECT * FROM tour_package WHERE city LIKE '%".$row_country2['id']."%' ORDER BY id DESC";
                    }

                }
                $rs=mysqli_query($con,$query);
                while($row = mysqli_fetch_array($rs)){
                  $tempCityF = preg_split ("/[\s;]+/", $row['city']);

                
                 for($xu=0; $xu<count($tempCityF);$xu++){
                  $querycityF = "SELECT * FROM city WHERE id=".$tempCityF[$xu];
                  $rscityF=mysqli_query($con,$querycityF);
                  $rowcityF=mysqli_fetch_array($rscityF);

                  if($rowcityF['id']==$_POST['name']){
                   $price2 = 999999999999;
                  ...
                  $tempCity = preg_split ("/[\s;]+/", $row['city']);
                  $tempCountry = preg_split ("/[\s;]+/", $row['country']);
                  $tempString = "";
                  $tempString2 = "";  
                ...
                <td>".$tempMinPerson."</td>
                <td>Rp ".number_format($sellingprice, 0, ".", ".")."</td>
                <td>Rp ".number_format($price2, 0, ".", ".")."</td>
