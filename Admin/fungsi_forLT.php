...
function get_hotel_forLT($datareq)
{
    ...
    $master_id = isset($datareq['master_id']) ? $datareq['master_id'] : '';
    $copy_id = isset($datareq['copy_id']) ?  $datareq['copy_id'] : '';
    $sfee_id = isset($datareq['sfee_id']) ? $datareq['sfee_id'] : '';
    $grub_id = isset($datareq['grub_id']) ? $datareq['grub_id'] : '';

    $query_hotel_data = "SELECT * FROM LT_AH_ListHotel WHERE copy_id='" . $copy_id . "' && master_id='" . $master_id . "' && grub_id='".$grub_id."'";
    ...
    return json_encode(array("adt" => $gt, "chd" => $gt, "inf" => $gt, "sgl" => $gt, "detail" =>$query_hotel_data), true);
}
