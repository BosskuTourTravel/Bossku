                ...
                <div class="card-header">
                    ...
                    <div class="card-tools">
                        <div class="input-group input-group-sm">
                            ...
                            <a class="btn btn-warning btn-sm tip" onclick="MP_Package(0,0,0)" title="Back" style="margin: 2px;"><i class="fas fa-arrow-left"></i></a>
                                ...
                                <a class="btn btn-primary btn-sm tip" onclick="MP_Package(2,<?php echo $_POST['id'] ?>,0)" title="Reload" style="margin: 2px;"><i class="fas fa-sync-alt"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body table-responsive p-0">
                    ...
                    <div style="padding: 20px;">
                        <table id="tokopedia" class="table table-striped table-bordered" style="width:100%; font-size: 12px;">
                            ...
                            <tbody>
                                <?php
                                $query = "SELECT * FROM Upload_tokopedia where mp_id='" . $_POST['id'] . "' ORDER BY id ASC limit 300";
                                    ...
                                    <div class="form-group">
                                        ...
                                        <datalist id="paket_list">
                                            ...
                                            foreach ($result as $data) {
                                                if ($data['grub_id'] != NULL) {
                                                    $judul = $data['judul'];
                                                    if ($data['change_judul'] != null) {
                                                        $judul = $data['change_judul'];
                                                    }
                                                    // echo $row['id']." ". $row['grub_id']." ". $row['grub_id']." ".$judul." ". $row['grub_name']." by ". $row['staff']."</br>";
                                            ?>
                                                    <option label="<?php echo $data['landtour'] . " " . $data['grub_name'] . " by " . $data['staff'] . " " . $data['id'] . "-" . $data['master_id'] . "-" . $data['grub_id'] ?>" value="<?php echo $judul ?>" data-id="<?php echo $data['id'] . "-" . $data['master_id'] . "-" . $data['grub_id']  ?>">
                                                <?php
                                                }
                                            }
                                                ?>
...
<script type="text/javascript">
    ...
    function add_package(x) {
        ...
        // alert(nama);
        $.ajax({
            url: "insert_paket_tokopedia.php",
            method: "POST",
            asynch: false,
            data: {
                paket: paket,
    ...
    function update_pack(x) {
        ...
        if (r == true) {
            $.ajax({
                url: "update_price_tokopedia.php",
                method: "POST",
                asynch: false,
                data: {
                    id: x
