...
foreach ($query_include as $check) {
    if ($check != '1' && $check != '15' && $check != '17' && $check != '32' && $check != '55') {
        ...
        if (($show_tps)) {
            ...
            // if($check =='8'){
            // var_dump($nol." ".$gt_chck_adt." > ".$result_tps['adt']."</br>");
            // }
        }
...
$total_manual_adt =  intval($result_profit['adt']) + intval($result_hp['twn']) + $fee_tl + $gt_chck_adt + intval($_POST['ltwn']);
$total_manual_chd =  intval($result_profit['chd']) + intval($result_hp['cnb']) + $fee_tl + $gt_chck_chd + intval($_POST['ltwn']);
$total_manual_inf =  intval($result_profit['inf']) + intval($result_hp['inf']) + $fee_tl + $gt_chck_inf + intval($_POST['ltwn']);
$total_manual_sgl =  intval($result_profit['adt']) + intval($result_hp['sgl']) + $fee_tl + $gt_chck_sgl + intval($_POST['ltwn']);

// echo $result_profit['adt'] . " - " . $result_hp['twn'] . " - " . $fee_tl . " - " . $gt_chck_adt . " - " . $_POST['ltwn'];
        ...
        <div style="padding: 20px;">
            <div class="row">
                ...
                if ($row_main['img1'] != "") {
                    ...
                    // var_dump("https://drive.google.com/thumbnail?id=".$thumbnail_gmb1);
                }
                if ($row_main['img2'] != "") {
                ...
                <table class="table table-bordered table-sm" style="border-color: black;">
                    ...
                    <tbody>
                        ...
                        ?>
                            ...
                            <td><?php echo "Rp." . number_format($_POST['twn'], 0, ",", ".") ?></td>
                            <td><?php echo "Rp." . number_format($_POST['sgl'], 0, ",", ".") ?></td>
                            <td><?php echo "Rp." . number_format($_POST['cnb'], 0, ",", ".") ?></td>
                            <td><?php echo "Rp." . number_format($_POST['inf'], 0, ",", ".") ?></td>
                            <td></td>
                        <?php
                        } else if ($_POST['ltwn'] != "") {
                        ?>
