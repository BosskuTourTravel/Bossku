...
function get_hotel_all($data)
{
    ...
    $bonus = number_format($row_itin2['pax_b']);
    $pax = "";
    $pax .= $row_itin2['pax'];
    ...
    return json_encode(array("pax" => $pax, "twn" => $atwn, "cnb" => $acnb, "inf" => $ainfant, "sgl" => $asgl, "id_hotel" => $id_hotel, "hotel" =>  $arr_hotel), true);
}

function get_hotel_one($x)
{
    include "../db=connection.php";

    $query_itin = "SELECT * FROM  LT_itinnew where id=" . $x['hotel_id'];
    $rs_itin = mysqli_query($con, $query_itin);
    $row_itin = mysqli_fetch_assoc($rs_itin);
    $arr_hotel = [];
    // array_push($arr_hotel,$query_itin);
    ...
    // $ceklist = $agent_twn." * ".$pr."/100 + ".$agent_twn." + ".$nom;
