...
function showImage2(x){
  // Get the modal
  var modal = document.getElementById("myModal2"+x);

  // Get the image and insert it inside the modal - use its "alt" text as a caption
  var img = document.getElementById("myImg2"+x);
  var modalImg = document.getElementById("img2"+x);
  img.onclick = function(){
    var captionText = document.getElementById("caption2"+x);
    modal.style.display = "block";
    modalImg.src = this.src;
    captionText.innerHTML = this.alt;
  }
          ...
          <div class='col-12'>
            <div class='card'>
                ...
                <div class='card-tools'>
                  <div class='input-group input-group-sm' style='width: 150px;'>
                    ...
                    <div class='input-group-append'>
                      <button type='submit' onclick='reloadManual(1,0,0)' class='btn btn-default'><i class='fa fa-arrow-left'></i></button>
              ...
              <div class='card-body table-responsive p-0'>";
                ...
                $query = "SELECT * FROM payment_detail_performaflight WHERE invoice_id = ".$_POST['id']." ORDER BY payment_number ASC";
                ...
                <th>Rekening Dibayarkan</th>
                <th>Lampiran Invoice</th>
                <th>Bukti Pembayaran Foto</th>
                <th>Bukti Pembayaran Text</th>
                <th>Upload Bukti Pembayaran</th>
                <th>Staff</th>
                <th>Pilihan Staff Com</th>
                <th>Option</th>
                </tr>
                </thead>
                <tbody id='myTable'>";
                while($row=mysqli_fetch_array($rs)){
                  ...
                  $querybank = "SELECT * FROM bank WHERE id=".$row['bank_dibayarkan'];
                  $rsbank = mysqli_query($con,$querybank);
                  $rowbank = mysqli_fetch_array($rsbank);

                  $queryagent = "SELECT * FROM agent WHERE id=".$row['agent'];
                  ...
                  <td>".$tempCount."</td>
                  <td>".$rowagent['company']." ( ".$rowagent['name']." )</td>
                  <td>Rp ".number_format($row['total_pembayaran'], 0, ".", ".")."</td>
                  <td>".$row['payment_number']."</td>
                  <td>".$row['tanggal_pembayaran']."</td>
                  
                  <td>".$row['payment_type']."</td>
                  <td>Rp ".number_format($row['total_dibayarkan'], 0, ".", ".")."</td>
                  <td>".$rowbank['short']." ( ".$rowbank['nama']." )</td>
                  <td>".$row['rekening_pembayaran']. " ( ".$row['atas_nama_pembayaran']." - " .$row['bank_pembayaran']." )</td>
                  <td><a href='https://2canholiday.com/".$row['img_lampiran']."' target='_blank'>Download</a>
                  <div id='myModal2".$row['id']."'  class='modal' data-backdrop='dynamic'>
                  <span id='closeModal2".$row['id']."' onclick='closeImage2(".$row['id'].")' class='close'>&times;</span>
                  <img  class='modal-content' id='img2".$row['id']."'>
                  <div id='caption2".$row['id']."'></div>
                  </div></td>";
                  echo "<td><img id='myImg".$row['id']."' onclick='showImage(".$row['id'].")' src='https://2canholiday.com/".$row['img_bukti_bayar']."' style='height:150px;width:100px;'>
                  ...
                  if($_SESSION['type']==1 or $_SESSION['type']==2){
                    echo "<td><button type='submit' onclick='insertPayment(-6,".$row['id'].",".$row['invoice_id'].")' class='btn btn-success'><i class='fa fa-plus'></i></button></br></br></td>";
                  }else{
                    echo "<td></td>";
                  }
...
<script>
  function delPaymentDetail(x,y){
    ...
    if (r == true) {
     $.ajax({
        url:"delPaymentDetailPerformaFlight.php",
        method: "POST",
        asynch: false,
        data:{id:x},
