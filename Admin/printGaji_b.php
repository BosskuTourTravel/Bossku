...
if($gj=='1'){
  ...
  //$querycomvp = "SELECT SUM(staff_com) as subvpcom FROM invoiceVisaPassport WHERE staff_id=".$rows['nama']." AND YEAR(stamp)=".$rows['yea']." AND MONTH(stamp) =".$rows['mon'];
  $queryfl = "SELECT SUM(adt) as sadt ,SUM(chd) as schd,SUM(inf) as sinf,SUM(adt_tax) as adtx,SUM(chd_tax) as chdx,SUM(inf_tax) as infx,SUM(staff_com) as subflcom,SUM(adt_price) as seladt,SUM(chd_price) as selchd,SUM(inf_price) as selinf,SUM(selling_adt_price) as subsap,SUM(selling_chd_price) as subchd,SUM(selling_inf_price) as subinf FROM flight  WHERE staff_id=".$rows['nama']." AND YEAR(stamp)=".$rows['yea']." AND MONTH(stamp) =".$rows['mon'];
//$queryfl = "SELECT SUM(staff_com) as subflcom FROM flight WHERE staff_id=".$rows['nama']." AND YEAR(stamp)=".$rows['yea']." AND MONTH(stamp) =".$rows['mon'];
}else{
  ...
  $queryfl = "SELECT SUM(adt) as sadt ,SUM(chd) as schd,SUM(inf) as sinf,SUM(adt_tax) as adtx,SUM(chd_tax) as chdx,SUM(inf_tax) as infx,SUM(staff_com) as subflcom,SUM(adt_price) as seladt,SUM(chd_price) as selchd,SUM(inf_price) as selinf,SUM(selling_adt_price) as subsap,SUM(selling_chd_price) as subchd,SUM(selling_inf_price) as subinf FROM flight WHERE stamp >= '".$tgaw."' AND stamp < '".$tgak."' AND staff_id=".$rows['nama'];
  //$queryfl = "SELECT SUM(staff_com) as subflcom FROM flight WHERE stamp >= '".$tgaw."' AND stamp < '".$tgak."' AND staff_id=".$rows['nama'];
}
////////////////////////////////////
...
/////////////////////////////////////
$flight_jual=($rowfl['sadt'] * $rowfl['subsap'])+($rowfl['schd'] * $rowfl['subchd'])+($rowfl['sinf'] * $rowfl['subinf']);
$flight_adt=$rowfl['sadt']*($rowfl['seladt']+$rowfl['adtx']);
$flight_chd=$rowfl['schd']*($rowfl['selchd']+$rowfl['chdx']);
$flight_inf=$rowfl['sinf']*($rowfl['selinf']+$rowfl['infx']);
$flight_total=$flight_adt+$flight_chd+$flight_inf;
$flight_nett = $flight_jual - $flight_total;
$flight_com=$rowfl['subflcom'] * '40'/'100';
...
$pdf->cell(40,6,'Rp.'.$rowlp['nominal'] .$plus,1,0);
$pdf->cell(10,6,'x',1,0,'C');
$pdf->cell(30,6,$rowlem['subdurasi'],1,0,'C');
...
$pdf->cell(30,6,number_format($flight_jual, 0, ".", "."),1,0);
$pdf->cell(30,6,number_format($flight_nett, 0, ".", "."),1,0);
$pdf->cell(15,6,'20 %',1,0,'C');
$pdf->cell(15,6,'40 %',1,0,'C');
$pdf->cell(10,6,'Rp',0,0,'C');
$pdf->cell(40,6,number_format($flight_com, 0, ".", "."),1,0,'C');
$ya = $ya+$jrow;
$pdf->setFont('Arial','',9);
$pdf->setXY(10,$ya);
...
#5
///////total///////////
$total=$rows['gaji'] + $rowjd['subjobdesk'] + $rows['bpjs'] + $jp + $rowlem['subtotal'] + $rowlem2['subtotal2'] + $rowcomvp['subvpcom'] + $rowcomlt['substaffcom'] + $flight_com;
/////////////////////
$ya = $ya+$jrow;
$pdf->setFont('Arial','',9);
$pdf->setXY(10,$ya);
...
$y=$rows['mon'];
$tgaw=date("$x-$y-10");
$tgak=date("Y-m-10", strtotime("+1 months",strtotime($tgaw)));
# header
$pdf->cell(180, 25,'',1,0);
$pdf->setFont('Arial','',14);
$pdf->text(70,20,'PT. PERFORMA TOUR & TRAVEL');
...
if($gj=='1'){
  ...
  $querycomlt = "SELECT SUM(staff_com) as substaffcom FROM invoice WHERE staff_id=".$rows['nama']." AND YEAR(stamp)=".$rows['yea']." AND MONTH(stamp) =".$rows['mon'];
  $querycomvp = "SELECT SUM(staff_com) as subvpcom FROM invoiceVisaPassport WHERE staff_id=".$rows['nama']." AND YEAR(stamp)=".$rows['yea']." AND MONTH(stamp) =".$rows['mon'];
  $queryfl = "SELECT SUM(staff_com) as subflcom,SUM(adt_price) as adt,SUM(chd_price) as chd,SUM(inf_price) as inf,SUM(selling_adt_price) as subsap,SUM(selling_chd_price) as subchd,SUM(selling_inf_price) as subinf FROM flight WHERE staff_id=".$rows['nama']." AND YEAR(stamp)=".$rows['yea']." AND MONTH(stamp) =".$rows['mon'];
}else{
  $querytjp = "SELECT COUNT(nominal) as cnom FROM tunjangan_price WHERE tgl >= '".$tgaw."' AND tgl < '".$tgak."' AND nama=".$rows['nama'];
  $querylem = "SELECT SUM(total) as subtotal,SUM(over) as subover  FROM lembur WHERE tgl >= '".$tgaw."' AND tgl < '".$tgak."' AND staff=".$rows['nama'];
  $queryjd = "SELECT SUM(total) as subjobdesk FROM total_job WHERE date >= '".$tgaw."' AND date < '".$tgak."' AND id_staff =".$rows['nama'];
  $querycomlt = "SELECT SUM(staff_com) as substaffcom FROM invoice WHERE stamp >= '".$tgaw."' AND stamp < '".$tgak."' AND staff_id=".$rows['nama'];
  $querycomvp = "SELECT SUM(staff_com) as subvpcom FROM invoiceVisaPassport WHERE stamp >= '".$tgaw."' AND stamp < '".$tgak."' AND staff_id=".$rows['nama'];
  $queryfl = "SELECT SUM(staff_com) as subflcom,SUM(adt_price) as adt,SUM(chd_price) as chd,SUM(inf_price) as inf,SUM(selling_adt_price) as subsap,SUM(selling_chd_price) as subchd,SUM(selling_inf_price) as subinf FROM flight WHERE stamp >= '".$tgaw."' AND stamp < '".$tgak."' AND staff_id=".$rows['nama'];
}
////////////////////////////////////
$rstjp=mysqli_query($con,$querytjp);
$rowtjp = mysqli_fetch_array($rstjp);
...
$rowfl=mysqli_fetch_array($rsfl);
/////////////////////////////////////
$flight_jual=$rowfl['subsap']+$rowfl['subchd']+$rowfl['subinf'];
$flight_beli=$rowfl['adt']+$rowfl['chd']+$rowfl['inf'];
$flight_nett=$flight_jual-$flight_beli;
////////////////////////////////////
$pdf->setFont('Arial','',9);
$pdf->setXY(10,$ya);
$pdf->cell(40,6,'GAJI BASIC',1,0);
...
$pdf->cell(10,6,'Hours',1,0);
$pdf->cell(10,6,'Rp',0,0,'C');
$pdf->cell(40,6,number_format($rowlem['subtotal'], 0, ".", "."),1,0,'C');
$ya = $ya+$jrow;
#4
$pdf->setFont('Arial','',9);
$pdf->setXY(10,$ya);
...
#5
///////total///////////
$total=$rows['gaji'] + $rowjd['subjobdesk'] + $rows['bpjs'] + $jp + $rowlem['subtotal'] + $rowcomvp['subvpcom'] + $rowcomlt['substaffcom'] + $rowfl['subflcom'];
/////////////////////
$ya = $ya+$jrow;
$pdf->setFont('Arial','',9);
$pdf->setXY(10,$ya);
