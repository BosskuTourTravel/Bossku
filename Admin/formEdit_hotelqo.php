...
$query_tq = "SELECT DISTINCT  tour_pack FROM hotelquo";
          ...
          <div class='col-12'>
            <div class='card'>
              <div class='card-header'>
                <h3 class='card-title' style='font-weight:bold;'>Hotel Quotation Package</h3>
              ...
              <div class='card-body table-responsive p-0'>";
                ...
                $query = "SELECT * FROM hotelquo order by id ASC";
                ...
                while($row = mysqli_fetch_array($rs)){
                  ...
                  $harga2=round($harga,0);
                    ...
                    <td>".$row['durasi']." Day</td>
                    <td>".$row['pax']."</td>
                    <td>".$kurs2."</td>
                    <td>".number_format($foc, 0, ".", ".")."</td>
                    <td>".number_format($bonus, 0, ".", ".")."</td>
                    <td>".number_format($tl, 0, ".", ".")."</td>
                    <td><b>".number_format(($row['total']+$foc+$bonus+$tl), 0, ".", ".")."</b></td>
                    <td><b>".number_format(($row['total']+$foc+$bonus+$tl)/$row['pax'], 0, ".", ".")."</b></td>
                    <td>".$row['ket']."</td>
                    <td>
                    
                    <button type='submit' onclick='del_hotelquo(".$row['id'].",".$_POST['id'].")' style='font-size:13px;' class='btn btn-danger'><i class='fa fa-trash' aria-hidden='true'></i></button>
                    <button type='submit' onclick='edit_hotelquo(".$row['id'].",".$_POST['id'].")' style='font-size:13px;' class='btn btn-primary'><i class='fa fa-edit' aria-hidden='true'></i></button>
...
<script>
  function del_hotelquo(x,y){
    var txt;
    var r = confirm("Are you sure to delete?");
    if (r == true) {
     $.ajax({
        url:"del_hotelquo.php",
        method: "POST",
        asynch: false,
        data:{id:x},
