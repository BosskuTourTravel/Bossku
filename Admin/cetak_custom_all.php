...
$arr_sfee = explode(",", $_POST['val_id']);
$query_sfee = "SELECT * FROM  LTP_insert_sfee where id=" . $arr_sfee[0];
...
$query_cek_addhari = "SELECT  COUNT(hari) AS plus_hari FROM  LT_AH_Main where copy_id='" . $row_data['id'] . "' && master_id='" . $row_data['master_id'] . "' && grub_id='" . $row_sfee['id_grub'] . "'";
...
<body>
    ...
    for ($c = 1; $c <= $json_day; $c++) {
        ...
        $query_cek_hari = "SELECT  id ,hari FROM  LT_AH_Main where copy_id='" . $row_data['id'] . "' && master_id='" . $row_data['master_id'] . "' && grub_id='" . $row_sfee['id_grub'] . "' && hari='$c'";
    ...
    <div style="padding: 5px 20px; font-size: 12pt;">
        ...
        <div style="padding-bottom: 10px; font-weight: bold;">JUDUL :
            <?php
            $query_judul = "SELECT * FROM LT_change_judul WHERE copy_id='" . $row_data['id'] . "' && grub_id='" . $_GET['grub_id'] . "'";
            $rs_judul = mysqli_query($con, $query_judul);
            $row_judul = mysqli_fetch_array($rs_judul);
        ...
        <div>
            <table class="table table-bordered table-sm" style="border-color: black;">
                ...
                <tbody>
                    ...
                    <?php
                    $no=1;
                    foreach ($arr_sfee as $val) {

                        $query_sfee2 = "SELECT * FROM  LTP_insert_sfee where id=" . $val;
                        $rs_sfee2 = mysqli_query($con, $query_sfee2);
                        $row_sfee2 = mysqli_fetch_array($rs_sfee2);

                        $query_grub3 = "SELECT LTP_grub_flight.id,LTP_grub_flight.grub_name,LTP_insert_sfee.date_set,LTP_insert_sfee.id as sfee_id,LTP_insert_sfee.adt,LTP_insert_sfee.chd,LTP_insert_sfee.inf,LTP_insert_sfee.ket,LTP_insert_sfee.tgl as tgl_buat FROM LTP_grub_flight INNER JOIN LTP_insert_sfee ON LTP_grub_flight.id = LTP_insert_sfee.id_grub where LTP_grub_flight.id='" . $row_sfee2['id_grub'] . "' && LTP_insert_sfee.id='" . $val . "'";
                        $rs_grub3 = mysqli_query($con, $query_grub3);
                        $row_grub3 = mysqli_fetch_array($rs_grub3);
                        //  var_dump($query_grub3);
                        $query_gf = "SELECT * FROM LTP_grub_flight_value where grub_id='" . $row_grub3['id'] . "' order by id ASC";
                        ...
                        $adt = $adt + $row_grub3['adt'];
                        $chd = $chd + $row_grub3['chd'];
                        $inf = $inf + $row_grub3['inf'];
                        ...
                        //////////////////////////////////////////////////
                        $total_manual_adt =  intval($result_profit['adt']) + intval($result_hp['twn']) + intval($fee_tl) + intval($gt_chck_adt) + intval($_POST['ltwn']);
                        $total_manual_chd =  intval($result_profit['chd']) + intval($result_hp['cnb']) + intval($fee_tl) + intval($gt_chck_chd) + intval($_POST['ltwn']);
                        $total_manual_inf =  intval($result_profit['inf']) + intval($result_hp['inf']) + intval($fee_tl) + intval($gt_chck_inf) + intval($_POST['ltwn']);
                        $total_manual_sgl =  intval($result_profit['adt']) + intval($result_hp['sgl']) + intval($fee_tl) + intval($gt_chck_sgl) + intval($_POST['ltwn']);
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                <?php
                                $query_sfee_tgl = "SELECT tgl FROM LTP_tgl_sfee where sfee_id='" . $val . "' order by tgl ASC";
    ...
    <div style="padding: 5px 20px; font-size: 12px;">
        <div class="row">
            <?php
            $no = 1;
            foreach ($arr_sfee as $val_fe) {
                $query_sfee4 = "SELECT * FROM  LTP_insert_sfee where id=" . $val_fe;
                $rs_sfee4 = mysqli_query($con, $query_sfee4);
                $row_sfee4 = mysqli_fetch_array($rs_sfee4);

            ?>
                <div class="col-4">
                    <div>
                        <?php
                        $query2 = "SELECT * FROM LT_add_transport_baru where master_id='" . $row_data['master_id'] . "' && copy_id='" . $_GET['id'] . "' && grub_id='" . $row_sfee4['id_grub'] . "' order by hari ASC, urutan ASC";
                        $rs2 = mysqli_query($con, $query2);
                        $row2 = mysqli_fetch_array($rs2);
                        // var_dump($query2);
                        if (isset($row2['id'])) {
                        ?>
                            <table class="table table-bordered table-sm" style="border-color: black; font-weight: normal; font-size: 8pt;">
                                <thead>
                                    <tr>
                                        <th scope="col" style="text-align: center;">Transport <?php echo $no ?></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    $query = "SELECT * FROM  LT_add_transport_baru where master_id='" . $row_data['master_id'] . "' && copy_id='" . $_GET['id'] . "' && grub_id='" . $row_sfee4['id_grub'] . "'  order by hari ASC, urutan ASC";
