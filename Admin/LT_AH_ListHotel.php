...
$query_data = "SELECT * FROM LT_AH_Main WHERE id=" . $_POST['id'];
...
?>
<input type="hidden" name="add_hari" id="add_hari" value="<?php echo  $_POST['id'] ?>">
<input type="hidden" name="master_id" id="master_id" value="<?php echo  $row_sub['master_id'] ?>">
<input type="hidden" name="copy_id" id="copy_id" value="<?php echo  $row_data['copy_id']  ?>">
<input type="hidden" name="grub_id" id="grub_id" value="<?php echo  $_POST['grub_id']  ?>">
<input type="hidden" name="sfee_id" id="sfee_id" value="<?php echo  $_POST['sfee_id']  ?>">
                    ...
                    <div class="card-tools">
                        <div class="input-group input-group-sm" style="width: 150px;">
                            <div class="input-group-append" style="text-align: right;">
                                <a class="btn btn-warning btn-sm" onclick="AH_Package(0,<?php echo $row_data['copy_id'] ?>,<?php echo $_POST['grub_id'] ?>,<?php echo $_POST['sfee_id'] ?>)"><i class="fa fa-chevron-circle-left"></i></a>
                                <a class="btn btn-primary btn-sm" onclick="AH_Package(3,<?php echo $_POST['id'] ?>,<?php echo $_POST['grub_id'] ?>,<?php echo $_POST['sfee_id'] ?>)"><i class="fas fa-sync-alt"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <table class="table table-bordered table-sm">
                                        ...
                                        <tbody>
                                            <?php
                                            $query_hotel_data = "SELECT * FROM LT_AH_ListHotel WHERE copy_id='" . $row_data['copy_id'] . "' && master_id='" . $row_sub['master_id'] . "' && grub_id='".$_POST['grub_id']."' && hari='".$hari."'";
                                            ...
                                            ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a class="btn btn-danger btn-sm" onclick="delete_hotel(<?php echo $row_hotel_data['id'] ?>,<?php echo $_POST['id'] ?>,<?php echo $_POST['grub_id'] ?>,<?php echo $_POST['sfee_id'] ?>)"><i class="fas fa-trash"></i></a>
                                                    </td>
                                                </tr>
                                            <?php
                                                $no++;
...
<script>
    ...
    function add_rate(x, y) {
        ...
        var sfee_id = document.getElementById("sfee_id").value;
        var grub_id = document.getElementById("grub_id").value;
        var hari = document.getElementById("hari").value;
        var add_hari = document.getElementById("add_hari").value;
        $.ajax({
            url: "LT_AH_InsertHotel.php",
            method: "POST",
            asynch: false,
            data: {
                master: master,
    ...
    function delete_hotel(x, y, z,u) {
        var txt;
        var r = confirm("Are you sure to delete?");
        if (r == true) {
            $.ajax({
                url: "LT_AH_del_hotel.php",
                method: "POST",
                asynch: false,
                data: {
                    id: x
