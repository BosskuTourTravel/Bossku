          ...
          foreach ($object->getWorksheetIterator() as $worksheet) {
               ...
               if ($halaman == 1) {
                    for ($row = 2; $row <= $highestRow; $row++) {
                         ...
                         $visa = mysqli_real_escape_string($con, $worksheet->getCellByColumnAndRow(2, $row)->getValue());
                         $jenis = mysqli_real_escape_string($con, $worksheet->getCellByColumnAndRow(3, $row)->getValue());
                         $tipe = mysqli_real_escape_string($con, $worksheet->getCellByColumnAndRow(4, $row)->getValue());
                         $durasi = mysqli_real_escape_string($con, $worksheet->getCellByColumnAndRow(5, $row)->getValue());
                         ...
                         $sell_price = mysqli_real_escape_string($con, $worksheet->getCellByColumnAndRow(9, $row)->getValue());
                         $agent_name =  mysqli_real_escape_string($con, $worksheet->getCellByColumnAndRow(10, $row)->getValue());
                         ...
                         if ($kota != '' or $kota_emb != '' or $visa != '' or $jenis != '' or $tipe != '') {
                              $query_update = "SELECT * FROM Visa2 where kota like'%" . $kota . "%' AND kota_embessy like '%".$kota_emb."%' AND visa like '%".$visa."%' AND jenis='".$jenis."' AND tipe='".$tipe."' AND agent_name='".$agent_name."'";
                              $rs_update = mysqli_query($con, $query_update);
                              $row_update = mysqli_fetch_array($rs_update);
                              if ($row_update['id'] == "") {

                                   $sql = "INSERT INTO Visa2 VALUES ('','$date','$kota','$kota_emb','$visa','$jenis','$tipe','$durasi','$kurs','$emb_price','$agent_price','$sell_price','$agent_name','$alamat','$tlpn','$status')";
                                   ...
                                   $sql2 = "UPDATE Visa2 SET kota='".$kota."', kota_embessy='".$kota_emb."', visa='".$visa."',jenis='".$jenis."', tipe='".$tipe."', durasi='".$durasi."',kurs='".$kurs."', embassy_price='$emb_price', agent_price='$agent_price', sell_price='$sell_price',agent_name='".$agent_name."',alamat='".$alamat."', tlpn='".$tlpn."',status='1'  where id =".$row_update['id'];
