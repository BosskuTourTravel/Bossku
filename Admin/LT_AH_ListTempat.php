        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title" style="font-weight:bold;">Landtour ADD Hari List Tempat</h3>
                    <div class="card-tools">
                        <div class="input-group input-group-sm" style="width: 150px;">
                            <div class="input-group-append" style="text-align: right;">
                                ...
                                <a class="btn btn-primary btn-sm" onclick="AH_Package(1,<?php echo $_POST['id'] ?>,<?php echo $_POST['grub_id'] ?>,<?php echo $_POST['sfee_id'] ?>)"><i class="fas fa-sync-alt"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body table-responsive p-0">
                    <div class="container" style="padding: 20px;">
                        <div class="list-tempat" id="list-tempat">
                            <?php
                            $statusTmp = 0;
                            $queryTmp = "SELECT * FROM  LT_AH_ListTempat where tour_id='" . $row_data['copy_id'] . "' && grub_id='" . $_POST['grub_id'] . "' && hari='$hari'";
                            ...
                            <form>
                                <div style="border: 2px solid; border-color:darkgreen; padding: 10px; margin-bottom: 5px;">
                                    ...
                                    <div id="dynamic_field">
                                        <?php
                                        if ($rowTmp['id'] != "") {
                                            $queryTmp2 = "SELECT * FROM LT_AH_ListTempat where tour_id='" . $row_data['copy_id'] . "' && grub_id='" . $_POST['grub_id'] . "' && hari='$hari'";
                                ...
                                ?>
                                    <div class="tmp-button-update" id="tmp-button-update">
                                        <button type="button" class="btn btn-success" id='but_upload4' onclick="fungsi_update_tmp(<?php echo $row_data['copy_id'] ?>,<?php echo $_POST['sfee_id'] ?>,<?php echo $_POST['grub_id'] ?>)">UPDATE</button>
                                    </div>
                                <?php
                                } else {
                                ?>
                                    <div class="tmp-button-add" id="tmp-button-add">
                                        <button type="button" class="btn btn-warning" id='but_upload3' onclick="fungsi_add_tmp(<?php echo $row_data['copy_id'] ?>,<?php echo $_POST['sfee_id'] ?>,<?php echo $_POST['grub_id'] ?>)">ADD</button>
                                    </div>
...
<script>
    function fungsi_add_tmp(x,y,z) {
        ...
        formData.append('grub_id', z);
        formData.append('hari', hari);
        $.ajax({
            type: 'POST',
            url: "insert_AH_ListTempat.php",
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    function fungsi_update_tmp(x,y,z) {
        var hari = $("input[name=hari]").val();
        let formData = new FormData();
        var values = $("input[name='tmp[]']").map(function() {
            return $('#tmp_list [value="' + $(this).val() + '"]').data('customvalue');
