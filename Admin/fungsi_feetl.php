...
function feeTL($datareq)
{
    ...
    if ($row_m['landtour'] != "undefined") {
        ...
        if(isset($row_sub['id'])){
            ...
            if(isset($row_cbng['id'])){
                $cabang = $row_cbng['nama'];
            }
        }
        ...
        if (isset($row_itin['id'])) {
            if ($row_itin['benua'] == "ASIA") {
                ...
                if ($pilih_negara != "") {
                    ...
                    $query_tl_fee = "SELECT * FROM  TL_fee where benua='" . $row_itin['benua'] . "' && negara='" . $pilih_negara . "' && mulai='" . $cabang . "'";
                    ...
                    $query_tl_fee = "SELECT * FROM  TL_fee where benua='" . $row_itin['benua'] . "' && negara != 'SINGAPORE' && negara !='MALAYSIA' && negara !='THAILAND' && mulai='" . $cabang . "'";
                    $rs_tl_fee = mysqli_query($con, $query_tl_fee);
                ...
                ////////////////////////////////////////// bukan asia
                $query_tl_fee = "SELECT * FROM  TL_fee where benua='" . $row_itin['benua'] . "' && mulai='" . $cabang . "'";
                $rs_tl_fee = mysqli_query($con, $query_tl_fee);
            ...
            return json_encode(array("p" => "sama","adt" => $vttpax, "chd" => $vttpax, "inf" => $vttpax, "sgl" => $vttpax, "detail" => "", "breakdown" => $arr_breakdown, "break_cost_tl" => $arr_break_costtl, "feetl_cover" => $arr_feetl_cover, "costtl_cover" => $arr_ct_cover, "grand" => "", "custom" => $ttpax), true);
        }
    }
}
...
function get_flight_tl($grub_id)
{
    ...
    while ($row_gfv_price_tl = mysqli_fetch_array($rs_gfv_price_tl)) {
        ...
        $query_ar_tl = "SELECT * FROM  LT_add_roundtrip where route_id='" .  $row_rd_tl['route_id'] . "'";
...
function feeTL_custom($datareq)
{
    ...
    if ($row_m['landtour'] != "undefined") {
        ...
        if ($tl_sfee != 0) {
            ...
            $v_sfee =  intval($tl_sfee) * $fee_hari;
        }
    }
    ...
    $transport =  $ferry + $flight + $train;
    $feeTL = intval($v_vt) + intval($v_meal) + intval($v_fee) + intval($v_sfee);
    $lt_cover = $pax_covery * $lt_price;
    $costTL = $lt_price + $lt_price_sub + $transport + $tips_flBagasi + $tips_flMeal + $tips_flTax;
    $val_cost_tl_fl = $flight +  $tips_flBagasi + $tips_flMeal + $tips_flTax;
    $feeTL_cover = $total_mealcus * -1;
