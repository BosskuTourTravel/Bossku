<?php
function ayoCurl($bank, $url, $td, $field1, $field2,$field3, $user_agent = "Googlebot/2.1 (http://www.googlebot.com/bot.html)")
{
    $url = $url;
    $ch = curl_init();
	...
	// ternyata website yd di curl sedang online
		...
		$kurs = preg_replace('/\s+/', '', $rows[$td][$field3]);
...
function curl(){
	...
	for ($x = 2; $x <= 17; $x++) {
		$data[] = ["bank" => "bca", "url" => "https://www.bca.co.id/Individu/Sarana/Kurs-dan-Suku-Bunga/Kurs-dan-Kalkulator",	"td" => $x, "field2" => "1", "field1" => "2", "field3" => "0"];
	}
	...
	for ($x = 2; $x <= 17; $x++) {
		$data[] = ["bank" => "mandiri", "url" => "https://www.bankmandiri.co.id/kurs", "td" => $x, "field2" => "1", "field1" => "2", "field3" => "0"];
	}
...
function bank(){
	...
	$data = array();
	foreach($curl as $cr){
		$url_kurs = 'https://https://www.2canholiday.com/Admin/getKurs.php?bank='.$cr['bank'];
		$data['data'][] = array(
			"bank" => $cr['bank'],
			"url_kurs" => $url_kurs
		);
	...
	$data['data'][] = array(
		"bank" => "all",
		"url_kurs" => 'https://https://www.2canholiday.com/Admin/getKurs.php?bank=all'
	);
...
function kurs($bank = ""){
	...
	foreach($curl as $cr){
		if($cr['bank'] == $bank){
			$data['data'][] = ayoCurl($cr['bank'], $cr['url'], $cr['td'], $cr['field1'], $cr['field2'],$cr['field3']);
		}else if($bank == "all"){
			$data['data'][] = ayoCurl($cr['bank'], $cr['url'], $cr['td'], $cr['field1'], $cr['field2'],$cr['field3']);
		}
	}	
