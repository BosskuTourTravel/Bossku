...
// $query_data = "SELECT * FROM  LTSUB_itin where id=" . $row_data['id'];
// $rs_data = mysqli_query($con, $query_data);
// $row_data = mysqli_fetch_array($rs_data);
...
foreach ($query_include as $check) {
    if ($check != '1' && $check != '15' && $check != '17' && $check != '32') {
        ...
        if ($check == '17') {
            ...
            $show_tps2 = get_adm_price($exclude);
            if ($show_tps) {
                $result_tps = json_decode($show_tps, true);
                ...
                array_push($arr_tmp, array("nama" =>  $result_tps['tmp'], "price" => $result_tps['adt']));
               
            }
            if ($show_tps2) {
                $result_tps2 = json_decode($show_tps2, true);
                array_push($arr_tmpex, array("nama" => $result_tps2['tmp'], "price" => $result_tps2['adt']));
            }
        }
        if ($check == '32') {
            $fee_on = 1;
...
$query_cek_addhari = "SELECT  COUNT(hari) AS plus_hari FROM  LT_AH_Main where copy_id='" . $row_data['tour_id'] . "' && master_id='" . $row_data['master_id'] . "' && sfee_id='" . $row_data['sfee_id'] . "'";
...
<body>
    ...
    for ($c = 1; $c <= $json_day; $c++) {
        ...
        $query_cek_hari = "SELECT  id ,hari FROM  LT_AH_Main where copy_id='" . $row_data['tour_id'] . "' && master_id='" . $row_data['master_id'] . "' && sfee_id='" . $row_data['sfee_id'] . "' && hari='$c'";
    ...
    <div style="padding: 5px 20px; font-size: 12pt;">
        ...
        <div>
            <table class="table table-bordered table-sm" style="border-color: black;">
                ...
                <tbody>
                    <?php
                    $query_master = "SELECT * FROM  LT_itinnew where kode='" . $row_data['landtour'] . "' order by pax ASC";
                    $rs_master = mysqli_query($con, $query_master);
                    $no = 1;
                    while ($row_master = mysqli_fetch_array($rs_master)) {
                        // var_export($row_master['agent_twn']);
                        ...
                        if ($show_feetl) {
                            $result_feetl = json_decode($show_feetl, true);
                            if ($fee_on == '1') {
                                if ($row_data['tl_pax'] != '0') {
                                    $fee_tl = $result_feetl['custom'] / $row_data['tl_pax'];
                                } else {
                                    $fee_tl = $result_feetl['adt'];
                                }
                            }
                        }
                        // echo "flight : ".$result_profit['adt']." + hotel : ".$result_hp['twn']." + include : ".$gt_chck_adt." + lain2 : ".$row_data['ltwn']." + Feetl : ".$fee_tl."</br>";
                        $total_manual_adt =  intval($result_profit['adt']) + intval($result_hp['twn']) + intval($fee_tl) + intval($gt_chck_adt) + intval($row_data['ltwn']);
                        $total_manual_chd =  intval($result_profit['chd']) + intval($result_hp['cnb']) + intval($fee_tl) + intval($gt_chck_chd) + intval($row_data['ltwn']);
                        $total_manual_inf =  intval($result_profit['inf']) + intval($result_hp['inf']) + intval($fee_tl) + intval($gt_chck_inf) + intval($row_data['ltwn']);
                        $total_manual_sgl =  intval($result_profit['adt']) + intval($result_hp['sgl']) + intval($fee_tl) + intval($gt_chck_sgl) + intval($row_data['ltwn']);
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="align-content: center; text-align: center;"><?php echo $start_date ?></td>
                            <td style="align-content: center; text-align: center;">
                                <?php
                                if ($row_data['tl_pax'] != '0') {
                                    echo $row_data['tl_pax'];
                                } else {
                                    echo $result_hp['pax'];
                                }
                                ?>
                            </td>
                            <td style="align-content: center; text-align: center;"><?php echo "Rp." . number_format($twn_rp['value'], 0, ",", ".") ?></td>
                            <td style="align-content: center; text-align: center;"><?php echo "Rp." . number_format($sgl_rp['value'], 0, ",", ".") ?></td>
                            <td style="align-content: center; text-align: center;"><?php echo "Rp." . number_format($cnb_rp['value'], 0, ",", ".") ?></td>
                            <td style="align-content: center; text-align: center;"><?php echo "Rp." . number_format($inf_rp['value'], 0, ",", ".") ?></td>
                        </tr>
                    <?php
                    $no++;
                    }
