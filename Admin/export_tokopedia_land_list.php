...
$query2 = "SELECT MP_tokopedia_land.id,MP_tokopedia_land.tgl,MP_tokopedia_land.nama,login_staff.name as staff FROM MP_tokopedia_land LEFT JOIN login_staff ON MP_tokopedia_land.staff=login_staff.id where MP_tokopedia_land.id='".$_GET['id']."'";
...
$query = "SELECT * FROM Upload_tokopedia_land where mp_id='" . $_GET['id'] . "' ORDER BY id ASC limit 300";
$rs = mysqli_query($con, $query);
$no = 1;
$sfee_id = 0;
$kode = "";

// $test = "lalalala yeyeye"."& CHAR(10) &"."asadadadadad";
while ($row = mysqli_fetch_array($rs)) {
    if($sfee_id ==  $row['urutan'] && $kode==$row['kode'] ){
...
header("Content-Disposition: attachment; filename=LT-" . $row2['nama'] . ".xls");
