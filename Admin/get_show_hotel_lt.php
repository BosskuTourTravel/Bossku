...
if (isset($_POST['negara'])) {
      if ($_POST['keyword'] != "") {
            $query_hotel = "SELECT DISTINCT name  FROM hotel_lt where country='" . $_POST['negara'] . "' && city='" . $_POST['city'] . "' && name LIKE '%" . $_POST['keyword'] . "%'  Order by rate_low  ASC";
            $rs_hotel = mysqli_query($con, $query_hotel);
      } else {
            $query_hotel = "SELECT DISTINCT name  FROM hotel_lt where country='" . $_POST['negara'] . "' && city='" . $_POST['city'] . "'  Order by rate_low  ASC";
            $rs_hotel = mysqli_query($con, $query_hotel);
      }
      while ($row_hotel =  mysqli_fetch_assoc($rs_hotel)) {
            $query_hotel_detail = "SELECT * FROM hotel_lt where country='" . $_POST['negara'] . "' && city='" . $_POST['city'] . "' && name='" . $row_hotel['name'] . "' Order by id ASC limit 1";
