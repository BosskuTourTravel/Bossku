...
$query_inc = "SELECT * FROM LT_include_master where  master_id='" . $row_data['id'] . "'";
...
while ($row_ex = mysqli_fetch_array($rs_ex)) {
    $cek_val  = array_search($row_ex['id'], $query_include);
    if ($cek_val == "") {
        // masuk exclude
...
<body>
    <div class="container" style="max-width: 2300px;">
        ...
        <?php
            ...
            $x++;
            $fee_tl += intval($_POST['tl_fee']);
            $meal_tl += intval($_POST['tl_meal']);
            $sfee_tl += intval($_POST['tl_sfee']);
        }
        ?>
            ...
            <table class="table table-bordered table-sm" style="border-color: black;">
                ...
                <tbody>
                    ...
                    while ($row_lt2 = mysqli_fetch_array($rs_lt2)) {
                        ...
                        $twin = ($agent_twn * $pr / 100) + $agent_twn + intval($_POST['lain']) + $fee_tl + $meal_tl + $sfee_tl + intval($_POST['tl_tlpn']);
                        $chd = ($agent_cnb * $pr / 100) + $agent_cnb + intval($_POST['lain']) + $fee_tl + $meal_tl + $sfee_tl + intval($_POST['tl_tlpn']);
                        $inf = ($agent_inf * $pr / 100) + $agent_inf + intval($_POST['lain']) + $fee_tl + $meal_tl + $sfee_tl + intval($_POST['tl_tlpn']);
                        $sgl = ($agent_sgl * $pr / 100) + $agent_sgl + intval($_POST['lain']) + $fee_tl + $meal_tl + $sfee_tl + intval($_POST['tl_tlpn']);
