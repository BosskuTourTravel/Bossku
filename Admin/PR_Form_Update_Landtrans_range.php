...
if ($_POST['z'] == '0') {
	...
	$query2 = "SELECT id FROM LTR_profit_range order by id ASC";
...
<div class="content-wrapper" style="width: 100%;">
		...
		<div class='col-12'>
			<div class="card">
				<div class="card-header">
					<div class="card-title">FROM Update Landtrans Profit</div>
					<div class="card-tools">
						<!-- <div class="input-group input-group-sm">
							<div class="input-group-append" style="text-align: left;">
								<div style="padding-right: 5px;">
									<button type="button" onclick="PR_Package(11,0,0)" class="btn btn-primary  btn-sm"><i class="fa fa-sync"></i></button>
								</div>
							</div>
						</div> -->
					</div>
					...
					<form>
						<div id="dynamic_field">
							...
							foreach ($data as $value) {
								$query = "SELECT * FROM LTR_profit_range where id='" . $value . "'";
								...
								<div class="row">
									...
									<div class="col" style="max-width: 200px;">
										<div class="form-group">
											<?php if ($u == 1) {
											?>
												<label for="tipe">Admin Tokped</label>
											<?php
											} ?>
											<input type="text" class="form-control form-control-sm" id="admin_tokped" name="admin_tokped[]" value="<?php echo $row['adm_tokped'] ?>">
										</div>
									</div>
									<div class="col" style="max-width: 200px;">
										<div class="form-group">
											<?php if ($u == 1) {
											?>
												<label for="tipe">Admin Shopee</label>
											<?php
											} ?>
											<input type="text" class="form-control form-control-sm" id="admin_shopee" name="admin_shopee[]" value="<?php echo $row['adm_shopee'] ?>">
										</div>
									</div>
									<div class="col" style="max-width: 200px;">
										<div class="form-group">
											<?php if ($u == 1) {
											?>
												<label for="tipe">Admin Blibli</label>
											<?php
											} ?>
											<input type="text" class="form-control form-control-sm" id="admin_blibli" name="admin_blibli[]" value="<?php echo $row['adm_blibli'] ?>">
										</div>
									</div>
									<div class="col" style="max-width: 200px;">
										<div class="form-group">
	...
	<script>
		function update_profit() {
			...
			var admin_tokped = $('input[name="admin_tokped[]"]').map(function() {
				return this.value; // $(this).val()
			}).get();
			var admin_shopee = $('input[name="admin_shopee[]"]').map(function() {
				return this.value; // $(this).val()
			}).get();
			var admin_blibli = $('input[name="admin_blibli[]"]').map(function() {
				return this.value; // $(this).val()
			}).get();
			...
			formData.append("admin_tokped", admin_tokped);
			formData.append("admin_shopee", admin_shopee);
			formData.append("admin_blibli", admin_blibli);
