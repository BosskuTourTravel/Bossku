...
$arr = [];
$arr_chck = json_decode($_POST['chck'], true);
...
if ($row_sub['landtour'] != "undefined") {
	...
	$arr_htl = [];
	$query_hotel = "SELECT * FROM  LT_select_PilihHTLNC where master_id='" . $row_sub['master_id'] . "'";
	$rs_hotel = mysqli_query($con, $query_hotel);
	$twin_h = 0;
	while ($row_hotel = mysqli_fetch_array($rs_hotel)) {
		$twin_h = $twin_h + $row_hotel['hotel_twin'];
		$arr_h = array(
			"master_id" => $row_hotel['master_id'],
			"copy_id" => $row_hotel['copy_id'],
			"hotel_name" => $row_hotel['hotel_name'],
			"hari" => $row_hotel['hari'],
			"hotel_twin" => $row_hotel['hotel_twin'],
			"hotel_triple" => $row_hotel['hotel_triple'],
			"hotel_family" => $row_hotel['hotel_family'],
			"status" => $row_hotel['status']
		);

		array_push($arr_htl, $arr_h);
	}
	$twn = $twn +  $twin_h + $l_twn;
	$sgl = $sgl + $twin_h + $l_sgl;
	$cnb = $cnb + 0 + $l_cnb;
	$inf = $inf + 0 + $l_inf;
}
$arr_dp_trans = [];
$query_plane = "SELECT * FROM LT_add_transport where master_id='" . $row_sub['master_id'] . "' && copy_id='" . $row_sub['id'] . "'  order by hari ASC,urutan ASC";
...
if (mysqli_query($con, $sql)) {
	...
	foreach ($arr_dp_trans as $val_tr) {
		$sql_tr = "INSERT INTO DP_add_transport VALUES ('','$date','$master_id','$copy_id','" . $row_id['id'] . "','" . $val_tr['type'] . "','" . $val_tr['hari'] . "','" . $val_tr['urutan'] . "','" . $val_tr['transport'] . "','" . $val_tr['status'] . "')";
