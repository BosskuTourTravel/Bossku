...
if ($uploadOk == 0) {
	...
	if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
		...
		$query_cek = "SELECT COUNT(*) as total FROM payment_detail WHERE paymentnumber=".$paymentnumber." AND invoice_id=".$invoiceid;
		$rs_cek = mysqli_query($con,$query_cek);
		$row_cek = mysqli_fetch_assoc($rs_cek);

		if($row_cek<=0){
			$sql = "INSERT INTO payment_detail VALUES ('',".$invoiceid.",".$paymentnumber.",".$paymentprice.",'".$description."','".$target_file2."','".$paymenttype."',".$paymentbank.",'".$tanggal."','".$stamp."',0)";
