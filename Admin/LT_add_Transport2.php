						...
						<div class="flight" id="flight">
							<form>
								...
								if ($row_data['landtour'] != "undefined") {
									$query_code = "SELECT * FROM flight_LTnew where tour_code ='" . $row_data['landtour'] . "'  order by tgl ASC, take ASC";
									$rs_code = mysqli_query($con, $query_code);
									$i = 1;
									while ($row_code = mysqli_fetch_array($rs_code)) {
										$detail = $row_code['maskapai'] . " " . $row_code['dept'] . " - " . $row_code['arr'] . " | " . $row_code['take'] . " - " . $row_code['landing'] . " | " . $row_code['rute'] . " | " . $row_code['type'];
								?>
										<div class="form-row" style="padding-bottom: 10px;">
											...
											<div class="col-md-2">
												...
												<input type="text" class="form-control  form-control-sm" id="tgl_fl<?php echo $i ?>" name="tgl_fl<?php echo $i ?>" value="<?php echo $row_code['tgl'] ?>" disabled>
											</div>
											<div class="col-md-6">
												<?php if ($i == '1') {
												?>
									...
									<div class="meal-button-add" id="meal-button-add">
										...
										<input type="hidden" id="kode_id" name="kode_id" value="<?php echo  $row_data['landtour'] ?>">
										<button type="button" class="btn btn-warning" onclick="fungsi_add_flight2()">ADD</button>
									</div>
								<?php
								} else {
								?>
									<div class="row">
										<div class="col-md-6">
											<select class="form-control form-control-sm" name="item" id="item" onchange="fungsi_item()">
												<option selected value="">Input Total Hari yang ada Flight Penerbangannya </option>
									...
									<div id="dynamic_field" style="padding-top: 10px;">
									</div>
									<div class="meal-button-add" id="meal-button-add">
										<input type="hidden" id="copy_id" name="copy_id" value="<?php echo  $row_data['id'] ?>">
										<input type="hidden" id="master_id" name="master_id" value="<?php echo  $row_data['master_id'] ?>">
...
<script>
	...
	function fungsi_kode(x) {
		var kode = $('#f_kode' + x).val();
		var v_kode = $('#f_kode_list' + x + ' [value="' + kode + '"]').data('customvalue');
		// alert(v_kode);
		$('#f_tgl' + x).empty();
		$.post('get_transport_list.php', {
			...
			var options = '';
			if (jsonData != '') {
				options += '<option data-customvalue="" value="Tanpa Tgl Penerbangan" selected></option>';
				for (var i = 0; i < jsonData.length; i++) {
					var counter = jsonData[i];
					// console.log(counter.tgl);
					options += '<option data-customvalue="' + counter.tgl + '" value="' + counter.tgl + '"></option>';
				}
				document.getElementById('f_tgl_list' + x).innerHTML = options;
			} else {
				options += '<option data-customvalue="" value="Tidak ada Penerbangan" selected></option>';
				document.getElementById('f_tgl_list' + x).innerHTML = options;
			}
	...
	function fungsi_tgl(x) {
		var kode = $('#f_kode' + x).val();
		var v_kode = $('#f_kode_list' + x + ' [value="' + kode + '"]').data('customvalue');

		var tgl = $('#f_tgl' + x).val();
		var v_tgl = $('#f_tgl_list' + x + ' [value="' + tgl + '"]').data('customvalue');
		$('#f_name' + x).empty();
		$.ajax({
			url: "get_transport_list.php",
			method: "POST",
			asynch: false,
			data: {
				'kode': v_kode,
			...
			success: function(data) {
				...
				var options = '';
				if (jsonData != '') {
					// options += '<option data-customvalue="" value="Pilih Flight Detail" selected></option>';	
					for (var i = 0; i < jsonData.length; i++) {
						var counter = jsonData[i];
						options += '<option data-customvalue="' + counter.id + '" value="' + counter.detail + '"></option>';
					}
					document.getElementById('f_name_list' + x).innerHTML = options;

				} else {
					options += '<option data-customvalue="" value="Tidak ada Detail Penerbangan" selected></option>';
					document.getElementById('f_name_list' + x).innerHTML = options;
				}
			}
		});
	...
	// 					var counter = jsonData[i];
	// 					// console.log(counter);
	// 					$('#f_name' + x).append('<option value=' + counter.id + '>' + counter.detail + '</option>');
	// 				}
...
<script>
	function fungsi_add_flight() {
		var jml = document.getElementById("item").options[document.getElementById("item").selectedIndex].value;
		var copy_id = $("input[name=copy_id]").val();
		var master_id = $("input[name=master_id]").val();

		let formData = new FormData();
		for (let i = 1; i <= jml; i++) {
			var kode = $('#f_kode' + i).val();
			var v_kode = $('#f_kode_list' + i + ' [value="' + kode + '"]').data('customvalue');

			var tgl = $('#f_tgl' + i).val();
			var v_tgl = $('#f_tgl_list' + i + ' [value="' + tgl + '"]').data('customvalue');

			var fname = $('#f_name' + i).val();
			var v_fname = $('#f_name_list' + i + ' [value="' + fname + '"]').data('customvalue');
			...
			if ((hari == "") || (urutan == "") || (v_tgl == "") || (v_fname == "")) {
				alert("Silakan isi semua form fields");
				return false;
			}
			formData.append('kode[]', v_kode);
			formData.append('hari[]', hari);
			formData.append('urutan[]', urutan);
			formData.append('tgl[]', v_tgl);
			formData.append('f_name[]', v_fname);
		}
	...
	function fungsi_add_flight2() {
		...
		if (cek != 0) {
			...
			alert("Silakan centang fields yang ingin di input !!");
		}
