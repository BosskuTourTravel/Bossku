        ...
        <div class="col-md-3" name="<?php echo $day ?>pil_transl<?php echo $loop ?>" id="<?php echo $day ?>pil_transl<?php echo $loop ?>" style="display: none;">
            ...
            <datalist id="<?php echo $day ?>transport_list<?php echo $loop ?>">
                ...
                while ($row_tr = mysqli_fetch_array($rs_tr)) {
                    $query_ag = "SELECT * FROM agent where id=".$row_tr['agent'];
                    $rs_ag = mysqli_query($con, $query_ag);
                    $row_ag = mysqli_fetch_array($rs_ag);
                ?>
                    <option data-customvalue="<?php echo $row_tr['id'] ?>" value="<?php echo $row_ag['name']." : ". $row_tr['city'] . " " . $row_tr['periode'] . " " . $row_tr['trans_type'] ?>"></option>
                <?php
                }
                ?>
            </datalist>
