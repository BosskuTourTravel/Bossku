...
if ($visa !== "") {
    ...
    $query_cek = "SELECT * FROM Visa_add where visa_id='$visa' && master_id='$master_id' && copy_id='$copy_id'";
    $rs_cek = mysqli_query($con, $query_cek);
    $row_cek = mysqli_fetch_array($rs_cek);
    if ($row_cek['id'] == "") {
        $query = "INSERT INTO Visa_add VALUES ('','$tgl','$master_id','$copy_id','$visa','$status')";
        ...
        $query = "UPDATE Visa_add SET visa_id='$visa' where id='" . $row_cek['id'] . "'";
        if (mysqli_query($con, $query)) {
            echo "success";
        } else {
            echo "gagal";
