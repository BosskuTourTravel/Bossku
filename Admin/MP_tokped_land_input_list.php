                    ...
                    <div class="card-tools">
                        <div class="input-group input-group-sm">
                            ...
                            <a class="btn btn-warning btn-sm tip" onclick="MP_Package(1,0,0)" title="Back" style="margin: 2px;"><i class="fas fa-arrow-left"></i></a>
                                ...
                                <a class="btn btn-primary btn-sm tip" onclick="MP_Package(7,<?php echo $_POST['id'] ?>,0)" title="Reload" style="margin: 2px;"><i class="fas fa-sync-alt"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body table-responsive p-0">
                    ...
                    <div style="padding: 20px;">
                        <table id="tokopedia" class="table table-striped table-bordered" style="width:100%; font-size: 12px;">
                            ...
                            <tbody>
                                <?php
                                $query = "SELECT * FROM Upload_tokopedia_land where mp_id='" . $_POST['id'] . "' ORDER BY id ASC limit 300";
                                    ...
                                    <div class="form-group">
                                        ...
                                        <datalist id="paket_list">
                                            ...
                                            foreach ($result as $data) {
                                                ...
                                                $pax = $data['pax'] . $pax_u . $pax_b;
                                            ?>
                                                    <option label="<?php echo  $pax. "pax : ".$data['hotel1'] ?>" value="<?php echo $data['landtour']. " ".$data['judul'] ." Paket ".$data['no_urut']." : ".number_format($data['agent_twn'])  ?>" data-id="<?php echo $data['landtour'] ?>" data-no="<?php echo $data['no_urut'] ?>" data-lt="<?php echo $data['id'] ?>">
                                                <?php
                                            }
                                                ?>
                                        </datalist>
...
<script type="text/javascript">
    ...
    function add_package(x) {
        ...
        var no = $("#paket_list option[value='" + $('#paket').val() + "']").attr('data-no');
        var lt = $("#paket_list option[value='" + $('#paket').val() + "']").attr('data-lt');
        // alert(paket+" "+no+" "+lt);
        $.ajax({
            url: "insert_paket_tokopedia_land.php",
            method: "POST",
            asynch: false,
            data: {
                paket: paket,
    ...
    function del_pack(x) {
        ...
        if (r == true) {
            ...
            $.ajax({
                type: 'POST',
                url: "MP_tokopedia_land_delete.php",
                data: formData,
                cache: false,
                processData: false,
                contentType: false,
    ...
    function update_pack(x) {
        ...
        if (r == true) {
            $.ajax({
                url: "update_price_tokopedia_land.php",
                method: "POST",
                asynch: false,
                data: {
                    id: x
