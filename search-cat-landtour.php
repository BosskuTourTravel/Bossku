        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            while ($row_cat = mysqli_fetch_array($rs_cat)) {
                ...
                if ($row_bn2['agent_twn'] != "") {
                    ...
                    $pax_val = $row_bn2['pax'] . $pax_u . $pax_b;
            ?>
                    <div class="bg-white shadow-md rounded-lg overflow-hidden flex flex-col">
                        <a href="<?php echo $domain_web ?>detail-landtour.php?id=<?php echo $row_bn2['id'] ?>&master=<?php echo $row2['id'] ?>" class="no-underline text-black">
                            <div class="relative">
                                <img src="<?php echo $domain_web ?>Admin/images/<?php echo $row2['gambar1'] ?>" class="w-full object-cover h-48 pt-12">
                                <?php if ($row_bn2['statuss'] == 'E') { ?>
                                    <div class="top-2 left-2 text-red-500 font-bold text-sm bg-white bg-opacity-70 px-3 py-2 rounded">
                                        EXPIRED
                                    </div>
                                <?php } ?>
                            </div>

                            <div class="px-4 py-2 flex-grow flex flex-col justify-between">
                                <div class="mb-2">
                                    <h3 class="font-bold text-lg"><?php echo $row2['judul'] ?></h3>
                                    <p class="text-gray-500 text-xs"><?php echo $row_bn2['kota'] ?></p>
                                    <p class="text-gray-500 text-sm"><?php echo $row2['landtour'] ?> - <?php echo $row_bn2['expired'] ?></p>
                                </div>
                                <div class="flex justify-between items-center">
                                    <div class="text-gray-500 text-sm"><?php echo $pax_val . " Pax"; ?></div>
                                    <div class="line-through text-purple-600 text-sm"><?php echo "IDR " . number_format($coret2, 0, ",", "."); ?></div>
                                </div>

                                <div class="flex items-start justify-between gap-2">
                                    <div class="text-green-600 text-sm flex flex-col items-start">
                                        <div>
                                            <i class="fa fa-building mr-1"></i>
                                            <i class="fa fa-bus mr-1"></i>
                                            <i class="fa fa-coffee"></i>
                                        </div>
                                        <div class="text-gray-500 text-sm leading-tight">SIC Everyday</div>
                                    </div>
                                    <div class="flex-1 text-right">
                                        <div class="text-green-600 text-lg font-semibold">
                                            <?php echo "IDR " . number_format($twn_rp2['value'], 0, ",", ".") ?>
                                        </div>
